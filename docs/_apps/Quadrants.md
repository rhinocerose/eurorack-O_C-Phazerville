...
Using the dedicated onboard audio codec hardware, the new Audio system provides customizable sound generation and effects processing chains in-the-box!

(TODO: screenshots of all Audio Applets)

### Preset Storage

Presets are stored internally in Flash using LittleFS, with 512KB of space allocated. There can be multiple Bank files, with 32 presets per Bank.

If a microSD card is detected, it becomes the primary storage for Bank files, using internal LFS as a fallback if a file is not found on load. Some utility functions for copying between LFS <-> SD card are planned.
